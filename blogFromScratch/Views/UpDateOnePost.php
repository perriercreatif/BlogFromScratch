...
<head>
  ...
  <title>Admin Scratch Modifier Post</title>
        ...
        <a href="?route=home">Home</a>
         - <a href="?route=postAdmin">Mes Posts</a>
           ...
           for ($i=0; $i< count($onePost); $i++){
             echo '<form action="?route=upDateOnePost&id='.$onePost[$i]->id.'" method="post">
               <div>
                 <label for="title">Titre du post</label> <br />
                 <input type="text" id="title" name="title" value="'.$onePost[$i]->title.'"/>
               </div>
               <div>
                 <label for="post"> - Corp du post - </label><br />
                 <textarea id="mytextarea" name="post" rows="20" cols="100">'.$onePost[$i]->post.'</textarea>
               </div>
               <div>
